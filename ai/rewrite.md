...
### Phase 4 — Fingerprints & similarity ✅ (2026-07-05)
...
**Accept:** criterion bench: 50k random fingerprints grouped < 1s; unit tests for known-similar image pairs (copy the fixtures from the Java repo's test resources). Delivered in `crates/dedup-core/src/fingerprint.rs` (MIME detection via `infer`/`mime_guess`; dHash with dihedral canonicalization; ffmpeg/ffprobe video temporal hash degrading gracefully when absent; `lopdf` PDF text hash; `symphonia` audio duration + BLAKE3 chunk hash) and `similar.rs` (LSH-banded `u64` grouping — exact for Hamming ≤ 3, i.e. similarity ≥ ~95 % — plus direct-scan video/pdf/audio grouping; `find_similar` reuses the Phase-3 sort order). Wired into `update.rs`; CLI `repo dupes --threshold <1-100>`. Tests: dHash rotation/mirror invariance and grouping unit tests, `tests/similar_repo.rs` (image pipeline) and `tests/video_repo.rs` (ffmpeg-gated), `benches/similar.rs` (50k random grouped in ~80 ms). The Java `ImageFingerprinterTest` fixtures are generated in-code (the Java repo ships no static image resources); dHash bit-values differ from the Java Graphics2D scaler but invariance and similarity semantics are preserved.

### Phase 5 — GUI shell (eframe) ✅ (2026-07-05)
`dedup` (no args) opens the window. Per the `### layout` override, the shell is a top tab bar (Repository / Duplicate / File management) with a settings cog; this phase fully implements the **Repository Management** tab (overview list with cached stats, add / delete / rename / relocate / duplicate, update/scan; `duplicate` is the ported `repo cp` — clone the index to a new repo at a new path) and leaves the Duplicate and File tabs as placeholders for Phases 6 and 7. Worker thread runs core operations; progress via `crossbeam_channel`; UI drains the channel each frame and keeps only the latest progress per repo (coalescing), `ctx.request_repaint_after(100ms)` while workers are active.
**Accept:** updating a 100k-file repo keeps the UI at interactive frame rates (no per-event repaint); cancel button stops within one file. Delivered in `crates/dedup-gui`: `theme.rs` (LCARS-inspired dark theme — black backdrop, amber text, rounded orange/lilac/periwinkle widgets), `worker.rs` (`ChannelProgress` adapter + `WorkerState` coalescing, unit-tested: 500 events collapse to one state entry; completion clears active), `app.rs` (`DedupApp` tabbed shell, `Arc<Store>` shared with worker threads, per-repo `CancellationToken`, deferred-action pattern to avoid double borrows; stats refreshed per-repo on completion so an in-flight update never contends on the same redb file). No per-event repaint (ChannelProgress only sends; UI polls at 10 Hz while active); cancel is the per-file `CancellationToken` check already covered by the Phase 2 tests. Verified by a live headless launch (window initialized and ran its event loop) plus the worker unit tests. Note: eframe/egui 0.35 reworked the API — `App::ui(&mut self, ui, frame)` instead of `update`, unified `Panel::top(..).show(ui, ..)` (old `TopBottomPanel`/`SidePanel` gone), `ctx.all_styles_mut` instead of `set_style`.

### Phase 6 — Duplicate review UI (the core screen) ✅ (2026-07-05)
...
**Accept:** reviewing 10k groups scrolls smoothly; deleting a 200-file batch is a single write txn per repo; nothing is deleted without an explicit confirm. Delivered per the `### layout` override as the **Duplicate Management** tab (`crates/dedup-gui/src/dupes_view.rs`): toggleable repo chips with a read-only (RO) flag (RO files are never preselected or deletable), a Duplicates/Similar selector with a threshold slider, and a paged group panel (50 groups/page) of file cards (thumbnail, path, repo, size, dimensions, mtime, KEEP/DELETE toggle with the best copy starred). Thumbnails: `dedup-core::thumbnail` writes `~/.cache/dedup/thumbs/<hex>.jpg` (≤512 px, keyed by content hash); `crates/dedup-gui/src/thumbs.rs` runs a background decode pool feeding an LRU of ≤200 egui textures, polled per frame. Deletes go through `dupes::delete_files` (explicit selection, batched `mark_missing` per repo in one txn) behind a mandatory confirm modal; "Auto-resolve rest" preselects every non-best copy in a deletable repo. Paging bounds the on-screen/thumbnailed groups to 50 so large result sets stay smooth. Covered by core tests (`thumbnail` gen/hex, `delete_files` explicit-selection) plus a live headless launch. Deferred to the later finetuning pass: keyboard navigation (arrows/space/enter) and the similar-search filter modal — the layout's per-mode filter button.

### Phase 7 — File ops + similarity UI, polish ✅ (2026-07-05)
...
**Accept:** feature-parity checklist against README command table complete; `verify-release`-style pass (fmt, clippy, tests, audit) green. Delivered per the `### layout` override as the **File Management** tab (`crates/dedup-gui/src/files_view.rs`): source/target repo chips (target chosen from the non-source repos), a COPY / MOVE / DELETE command selector, a mime/name/size filter, a `from → to` preview (first 30, with a total count), and a background-threaded run behind a confirmation modal with cancel — all on top of the existing `diff_copy`/`diff_delete` core ops (content compared by size + hash). Similarity search with the threshold slider was delivered in Phase 6's Duplicate Management tab (SIMILAR mode). Polish: `--ui-scale <0.5–3.0>` (top-level flag → `egui set_zoom_factor`). The UI is deliberately LCARS-dark (not system light/dark). `verify-release` pass: `cargo fmt`/`clippy -D warnings`/`cargo test` (73 tests) all green; `cargo audit` green via a documented `.cargo/audit.toml` ignoring unfixable transitive advisories from the eframe→winit→Wayland stack (quick-xml is build-time-only in wayland-scanner) and indicatif. Deferred finetuning: keyboard nav (Phase 6), the similar-search filter modal, and multi-source/multi-target file ops (currently single source + single target).
