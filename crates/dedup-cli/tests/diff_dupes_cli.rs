...
/// End-to-end sanitize workflow: scan a disk, copy its content that is unique
/// against the sanitized repo AND an already-processed disk, then mark the
/// source triage-done.
#[test]
fn sanitize_copies_uniques_against_references_and_marks_done() -> TestResult {
    let sb = Sandbox::new()?;

    // The disk to triage holds two distinct contents.
    sb.write("disk/f1.txt", b"one")?;
    sb.write("disk/f2.txt", b"two")?;
    sb.repo("disk")?;

    // A previously-processed disk already holds "one".
    sb.write("prior/p.txt", b"one")?;
    sb.repo("prior")?;

    // The sanitized destination starts empty.
    sb.repo("sanitized")?;

    sb.dedup()?
        .args(["sanitize", "disk", "sanitized", "--ref", "prior"])
        .assert()
        .success()
        .stdout(predicate::str::contains("Copied 1 unique"))
        .stdout(predicate::str::contains("Marked 'disk' triage-done"));

    // Only the content absent from every reference ("two") was copied.
    assert!(
        std::path::Path::new(&sb.path("sanitized/f2.txt")).exists(),
        "unique content copied into the sanitized repo"
    );
    assert!(
        !std::path::Path::new(&sb.path("sanitized/f1.txt")).exists(),
        "content already known to the prior disk is not copied"
    );
    Ok(())
}
