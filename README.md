...
- **File management** (implemented) — pick a source repo and a target repo, choose copy / move / delete, narrow with a mime/name/size filter, preview the first `from → to` transfers, and run it on a background thread behind a confirmation (copy/move transfer content the target lacks; delete removes source files the target already has). Content is compared by size + hash, never path. Running an operation keeps both repo indexes in sync as it proceeds — copies/moves are recorded in the target repo index and moved/deleted files are marked missing in the source — and shows live progress (a spinner, the file currently being handled, the last actions, and a running count) styled like the scan progress. PREVIEW and RUN are mutually exclusive: pressing RUN clears the preview and pressing PREVIEW clears the run log. For copy/move you can also set a relative subfolder inside the target: type a path in the **INTO** field or use **BROWSE** to pick (or create) a folder inside the target repo. Files keep their source-relative path under that subfolder (e.g. `photos/2020/a.jpg` into `imports/batch1` lands at `<target>/imports/batch1/photos/2020/a.jpg`); an empty value places them at the target root. A **DIFF** command turns the same tab into a manual two-repo comparison: pair the repos by content or by path, then resolve each difference yourself with per-row COPY / DELETE / RENAME / OVERWRITE buttons (a side holding several copies of the same content is narrowed down with DELETE ALL / KEEP 1 first). Every action is applied immediately to disk and both indexes, and the board re-compares after each one.
- **Sync groups** (implemented) — keep a repository backed up to one or more others natively, instead of duplicating and relocating a repo by hand. A group is one **main** repo plus its **sinks**, with a per-group mode: `ADD ONLY` copies content a sink lacks and never deletes, `MIRROR` also deletes sink content the main no longer has so the sink converges on exactly the main's content. Membership is editable (add a sink, take one out, promote a sink to main), a repo belongs to at most one group, and a member cannot be renamed or removed while it is in one. PREVIEW plans every sink into the shared review board; RUN SYNC confirms first and then pushes on a background thread, each sink independently, never changing the main.

Settings (cog, top-right) exposes the hashing thread count used by scans and a SHORT/VERBOSE toggle for hover-tooltip detail throughout the app. The thread count, tooltip verbosity, and the Duplicate tab's similarity threshold persist across launches (in `gui_settings.json` in the config dir); per-repo read-only state is intentionally not persisted (repos re-lock on every launch as a deletion safeguard). Settings also has an OPEN LOG FOLDER button: every run of the CLI or GUI writes a session log to `$XDG_STATE_HOME/dedup/logs` (`~/.local/state/dedup/logs` by default) and the ten most recent are kept, so a bug report can carry what actually happened. Launch with `--ui-scale <0.5–3.0>` to scale the interface (e.g. `dedup --ui-scale 1.25`). See the [GUI guide](docs/gui/index.md) for a full walkthrough with screenshots.
...
- [**CLI Reference**](docs/cli.md) — every command, in depth, with examples (multi-ref diff, filter syntax, the full command set).
- [**GUI Guide**](docs/gui/index.md) — a walkthrough of every tab and control, with screenshots: [Repositories](docs/gui/repositories.md), [Duplicates](docs/gui/duplicates.md) (lightbox, A/B compare, audio/video preview), [Files](docs/gui/files.md), [Sync Groups](docs/gui/sync-groups.md).
