...
Settings (cog, top-right) exposes the hashing thread count used by scans. The thread count and the Duplicate tab's similarity threshold persist across launches (in `gui_settings.json` in the config dir); per-repo read-only state is intentionally not persisted (repos re-lock on every launch as a deletion safeguard). Launch with `--ui-scale <0.5–3.0>` to scale the interface (e.g. `dedup --ui-scale 1.25`).
